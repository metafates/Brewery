...
macOS app: SwiftUI, Xcode 26 project, single scheme `Brewery`. No third-party dependencies. See `ARCHITECTURE.md` — it is the spec, not an afterthought: features are designed there first, each grounded in facts verified against the Homebrew source, then implemented — and the document is a current-state spec, rewritten in place as the system changes (no inline version numbering; history lives in git). Verify such facts against the brew source at `$(brew --repository)` — Homebrew installs itself as a full git checkout, so the Ruby this machine's brew actually runs already sits on disk under `Library/Homebrew/`; nothing to clone (without a local brew: `git clone https://github.com/Homebrew/brew`). Cite `file:line` rather than trusting memory, but grep the symbol first — line numbers drift between brew releases, so a citation landing a few lines off means the checkout moved, not that the claim is wrong (`git -C "$(brew --repository)" describe --tags` says which version you are reading). brew 6.x changed a lot (tap trust gate, services in core).
...
- `app.outlines["Sidebar"]` matches by AX *label*; the outline's `identifier` property is EMPTY, so `where o.identifier != "Sidebar"` filters nothing when enumerating outlines — and the sidebar always holds its section row selected, making naive "any row selected?" sweeps permanently true. Filter by column position (`o.frame.minX >= app.outlines["Sidebar"].frame.maxX`).
- A `Button` in a *List row* fuses its label into ONE AX element (`'homebrew/core, 8 564 formulae'`) with no staticText children — query such rows via `app.buttons` with a label-prefix predicate (grid/ScrollView buttons like PackageCard keep exposing inner staticTexts; List rows don't).
...
- **`typeKey` goes through the active keyboard layout.** With a non-English layout selected system-wide, *every* key equivalent silently misses — ⌘R, ⌘F, ⌘1…⌘9 — while clicks and menu-item clicks keep working, so the suite fails in a way that looks like a broken binding or a dead command. It reproduces identically on any commit, which is the tell. Diagnose by clicking the menu item the shortcut duplicates: if the click works and the key does not, switch the input source to English and re-run before touching code.
