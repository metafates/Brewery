...
nonisolated struct StateSnapshot: Codable {
    ...
    /// Optional top-level additions that decode as nil from old files (like `pinned`) need no
    /// bump — the catalog cache's tap-field rule. A mismatch reads as no snapshot —
    ...
    /// The pin-ledger scan's last answer; read back `?? []`.
