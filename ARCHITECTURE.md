...
- **(v10) FontPreviewTests**: the basename rule — `ttf/Hack-Regular.ttf` resolves to `<dir>/Hack-Regular.ttf` (fixture directory injected; the artifact name is source-relative, the installed file is not); missing file → nil / no faces; face extraction from a system `.ttc` (Helvetica — one file, several faces, non-empty family and PostScript names); pure ordering — duplicates by PostScript name collapse, weights ascend, italic follows its upright.
