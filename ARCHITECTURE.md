...
| `FontPreview.swift` *(v10)* | Installed-font face resolution: artifact name → `~/Library/Fonts` basename → CoreText descriptors; pure ordering/dedup |
...
| `PackageDetailView.swift` | Sheet: `DetailPage` per drill-down level (manual stack, footer back button, swipe-back monitor), header, links, caveats renderer (`CaveatFormat`/`CopyButton`; native `Text` since v9), Contents/Service sections, operation log |
...
Overlay refresh happens on launch, on ⌘R, and after every mutating operation completes (success or failure). **(v9)** A finished operation keeps its target busy until that post-operation refresh lands (`BrewOperation.awaitingRefresh`, consulted by `isBusy` — the `upgradeAll` fan-out included): between completion and the probes landing the overlays still answer for the old world, and dropping busy on `state` alone flashed the stale answer — "Install", a beat after installing — before the card settled on "Installed" (HIG *Feedback*: status reflects what is actually known; *Progress indicators*: keep them moving so people know something is continuing to happen). The card makes exactly one swap, spinner → result, through its existing `.blurReplace`/Reduce-Motion crossfade; `BusyThroughRefreshTests` pins the sequence. **(v2)** Each refresh also sweeps the receipts (`Receipts.swift`, `@concurrent` — ~350 small JSON reads, tens of ms): the keg read is the one whose version dir matches the *last* version `brew list` reported; `runtime_dependencies` `full_name`s are normalized to short names per the join rule. `AppModel` then inverts the dependency map once into `dependents: [Package.ID: [Package.ID]]` — "who depends on X" is a dictionary lookup, no graph machinery.
...
- **Grid**: `LazyVGrid(columns: [GridItem(.adaptive(minimum: 230))])`. Card: 44 pt icon, name (headline), status line — pills for identity (`Formula`/`Cask`/`Font` tag, **(v4)** tap-owner tag), then one `·`-joined run of plain text for version and state, the App Store metadata pattern (version — "1.2 → 1.3" in orange when outdated, cask comma-versions like `2.1.50,56f0a83` truncated at the comma — then "deprecated" in red / "pinned" / "dependency"; a bare value sandwiched between capsules read as clutter), 2-line description, **(v3)** a "Provides `<cmd>`" caption on command-matched search hits, trailing control — `Install` / `Update` (`.borderedProminent`) / an "Installed" label with a green `checkmark.circle.fill` (state wears no button chrome: a disabled button washes out and reads as broken; the card's hidden height-reserving twin is always the tallest variant so mixed rows stay flush) / a bare small `ProgressView` when busy **(v9)**: status only, no cancel affordance on the card. Two attempts put one there — first a separate ✕ beside the spinner (two controls where the platform draws one), then the App Store's stop-square-in-the-spinner — and both read as clutter at card size, so the card settled on what macOS prefers for background work (HIG *Progress indicators*, macOS: "prefer an activity indicator (spinner) to communicate the status of a background operation"). Halting stays one click away on the surfaces that own the queue — the operations popover's row and the log window's toolbar — which keeps "when it's feasible, let people halt processing" satisfied app-wide. `disabled` packages: button disabled with explanation in detail. The kind tag, the tap-owner tag and the detail pane's share one small `TagLabel`. **(v7)** `CardButtonStyle` gains a *selected* state — the accent border it already used for pressed (a press is a selection about to happen, so the card does not change costume between the two) plus an accent wash, added conditionally rather than as an always-present layer at `opacity(0)`, because 60 cards are rebuilt on every keystroke. **(v9)** Cards and service rows carry context menus (HIG *Context menus*: "support context menus consistently throughout your app" — the tap rows already did): the state's own action (Install / Update / Open / Start / Stop), Open Homepage, Copy Name (the brew token) — small, relevant, unavailable items absent rather than dimmed, and every item also present in the main interface. Installed single-bundle casks wear **Open** in the card's action slot — the App Store's grammar for an installed app — bordered, so the filled button stays the state-changing one; formulae and multi-app casks keep the Installed label. **(v10)** Installed font casks wear it too — their Open goes to Font Book, the pane's grammar on the card — and the context menu carries the same item.
...
- **(v3) Detail additions**, each section rendered only when its data exists, in this order after the header (v2's Dependencies / Required by follow them):
  ...
  - **(v10) Font preview** — a font cask's payload *is* its glyphs, so an installed font's pane shows them: a **Preview** section, first after the links row (for a font this is the screenshots slot, and the App Store puts screenshots before prose), one sample line per face rendered in the face itself — the pangram at `Font.custom(postScriptName, size: 18, relativeTo: .title3)` (relative, so it tracks text-size settings — HIG *Typography*: "implement accessibility features for custom fonts"), captioned with the style name (prefixed with the family when a pack ships several). Faces come from disk, not from the token: brew moves each font file to `fontdir` (default `~/Library/Fonts`, `cask/config.rb:26`; `Font < Moved`, `cask/artifact/font.rb`) under its **source basename** (`cask/artifact/relocated.rb:66-67` — the artifact *name* in the API is a source-relative path like `ttf/Hack-Regular.ttf`, so only its basename resolves), where the system has already registered it; `CTFontManagerCreateFontDescriptorsFromURL` on each surviving file yields real family/style/weight per face, `.ttc` collections included. The list is bounded and *representative*, never expanding: faces dedupe by PostScript name, each family orders Regular → Italic → heavier weights (upright before italic at equal distance from regular — never Thin first, which is both the least legible face and the one HIG *Typography* warns against leaning on), families interleave round-robin so a multi-family pack shows one row per family before any second faces, six show, and a quiet caption states the remainder ("And 90 more styles in Font Book") — no silent cap. A "Show N More Styles" expander shipped first and died in review: a Nerd Font pack inflated the pane by 96 rows, burying every section below the preview, and a hypothetical thousand-face pack has no inline answer at all. The full specimen is deliberately *not* this pane's job — an expander rebuilt a worse Font Book (the platform already ships one, reachable on the Open button), a popover big enough to hold it violates "avoid making a popover too big", and a specimen window is an app inside the app. Resolution is `@concurrent` (CoreText reads the files) and runs once per page, not per pass; a file missing from `~/Library/Fonts` simply contributes no face, and a section with no faces never renders — the same honest degrade as Open. *ponytail*: a custom `--fontdir` install resolves nowhere and gets no preview; reading the receipt's cask config would lift that. VoiceOver reads each row as its face name, never the pangram. The action row gains **Open** — the same word and chrome as an app cask's, because the payload defines what opening means (double-clicking a font file in Finder opens Font Book the same way; an "Open in Font Book" title was tried and looked like a second grammar in the one slot, and Quick Look's "Open with Font Book" wording was likewise dropped), with the destination in the tooltip; the resolved file goes to `NSWorkspace` — LaunchServices, no brew, safety model untouched. The HIG push-button ellipsis rule ("append a trailing ellipsis when a push button opens another window, view, or app") deliberately does not apply, on the App Store's own precedent: launching the payload *is* the action, not a step toward one. The same pass moved the action row from top alignment to **`.firstTextBaseline`** — the Installed label's text hung above the bordered button beside it when the row aligned boxes instead of text, a defect the app-cask Open had quietly shipped too. **Installed casks only, deliberately**: an uninstalled font's files sit inside a remote zip or dmg, and fetch-unpack-mount for a preview is machinery this pane refuses — the homepage link stays the pre-install preview. No `Package` shape change, no cache bump.
...
- **Menu bar** (**v7** — every toolbar action needs a menu bar command and a key equivalent; before this only three commands existed and no destination had a keyboard path). Order follows the standard anatomy: Brewery, Edit, View, Homebrew, Window, Help.
  ...
  - Selection and the two pane/popover visibility flags live in `AppModel`, not view `@State`, for the same reason the per-section queries do: a `Commands` builder can only reach app-level state. **(v9)** The sidebar selection persists across launches (`sidebar.section`) alongside the inspector's visibility — HIG *Launching*: "restore the previous state when your app restarts so people can continue where they left off."
...
- **(v10) FontPreviewTests**: the basename rule — `ttf/Hack-Regular.ttf` resolves to `<dir>/Hack-Regular.ttf` (fixture directory injected; the artifact name is source-relative, the installed file is not); missing file → nil / no faces; face extraction from a system `.ttc` (Helvetica — one file, several faces, non-empty family and PostScript names); pure representative selection — duplicates by PostScript name collapse, a single family orders Regular/Italic/Bold/Bold Italic, families round-robin (every family's Regular before any family's second face), the cap and the dropped count hold, and under-limit packs drop nothing.
