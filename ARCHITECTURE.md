...
- **Grid**: `LazyVGrid(columns: [GridItem(.adaptive(minimum: 230))])`. Card: 44 pt icon, name (headline), status line (a `Formula`/`Cask`/`Font` tag, then version; "1.2 → 1.3" in orange when outdated, cask comma-versions like `2.1.50,56f0a83` truncated at the comma for display; "deprecated" in red), 2-line description, **(v3)** a "Provides `<cmd>`" caption on command-matched search hits, trailing button — `Install` / `Update` (`.borderedProminent`) / checkmark (installed, disabled) / a `ProgressView` plus a stop button (busy, and only when that package has its own operation — a card made busy by Upgrade All has nothing of its own to stop). `disabled` packages: button disabled with explanation in detail. The kind tag, the dependency tag and the detail sheet's share one small `TagLabel`.
