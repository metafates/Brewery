...
- **Detail**: `.sheet(item: $selectedPackage)` — App Store-like, keeps grid scroll position. Large icon, name + kind tag, version(s), deprecation/disabled banner, homepage `Link` beside a source `Link` (the package's `.rb` on GitHub, labelled with the monospaced file name — built from the API's tap-relative `ruby_source_path` + the kind's repo, never from a guessed sharding scheme; synthesized packages have no path and no link), action button, and the package's latest operation log if any. **(v2)** Two more sections when installed: **Dependencies** — the receipt's installed runtime deps as tappable rows (icon, name, installed version; `declared_directly` ones sorted first); **Required by** — the inverted map's entries for this package (present for any depended-on item, which is also how a dependency-only item explains why it exists). Tapping a row **pushes** onto the sheet's lightweight manual stack (v5.x): each page opens at the top (fresh ScrollView identity per package), a named back button bottom-left in the constant-height footer ("‹ openssl@3" — a bar above the content skewed the layout whenever it toggled) + ⌘[ pop with a directional slide, and a two-finger swipe right pops too (local scroll-event monitor, latched per gesture, honoring the system swipe setting — the App Store gesture). Manual rather than `NavigationStack` because this custom fixed-size sheet (own footer, content-derived height, focus hacks) would fight framework navigation chrome. Escape still closes the sheet.
...
- **Newcomer explanations** (v6.x) — the app assumes no Homebrew knowledge, in three Apple grammars: a one-time dismissible **TipKit** card atop Discover explains formulae vs casks (the two words the store runs on; `Tips.configure()` at launch persists the dismissal); **header subtitles** on the Taps and Services lists (caption under the section title — orientation copy must be read *before* the rows; a footer teaches you what a list was only after you've read it) say in plain words what a tap and a service are; and every jargon **tag carries a `.help` tooltip** ("A command-line tool that runs in Terminal" / "A Mac app" / "A font" / the tap-owner explanation) via `TagLabel`'s optional help.
