...
- **Newcomer explanations** (v6.x) — the app assumes no Homebrew knowledge, in three Apple grammars: a one-time dismissible **TipKit** card atop Discover explains formulae vs casks (the two words the store runs on; `Tips.configure()` at launch persists the dismissal); Settings-style **section footers** on the Taps and Services lists say in plain words what a tap and a service are; and every jargon **tag carries a `.help` tooltip** ("A command-line tool that runs in Terminal" / "A Mac app" / "A font" / the tap-owner explanation) via `TagLabel`'s optional help.
